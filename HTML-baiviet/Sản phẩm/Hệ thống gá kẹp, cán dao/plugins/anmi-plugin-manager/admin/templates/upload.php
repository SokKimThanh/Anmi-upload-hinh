...
<div class="wrap anmi-pm-wrap">
	...
	<div class="anmi-upload-form">
		<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
			...
			<div class="form-field">
				...
				<span class="description"><?php printf(esc_html__('Maximum file size: %s', 'anmi-plugin-manager'), esc_html($max_upload)); ?></span>
			</div>
			...
			<div class="form-field">
				...
				<span class="description"><?php esc_html_e('Leave unchecked to review the plugin before activation.', 'anmi-plugin-manager'); ?></span>
			</div>
	...
	<div class="anmi-upload-info">
		<h3><?php esc_html_e('Staging workflow overview', 'anmi-plugin-manager'); ?></h3>
		<ol>
	...
	<?php if ($has_session): ?>
		<div class="anmi-upload-session">
			<h2><?php esc_html_e('Scan Results Ready for Staging', 'anmi-plugin-manager'); ?></h2>

			<div class="anmi-upload-meta">
				<div class="meta-card">
					<span class="meta-label"><?php esc_html_e('Uploaded file', 'anmi-plugin-manager'); ?></span>
				...
				<?php if (!empty($metadata)): ?>
					<div class="meta-card">
						<span class="meta-label"><?php esc_html_e('Plugin name', 'anmi-plugin-manager'); ?></span>
				...
				<?php if (!empty($metadata['plugin_file'])): ?>
					<div class="meta-card">
						<span class="meta-label"><?php esc_html_e('Plugin file', 'anmi-plugin-manager'); ?></span>
						<code><?php echo esc_html($metadata['plugin_file']); ?></code>
					</div>
				<?php endif; ?>

				<div class="meta-card">
					<span class="meta-label"><?php esc_html_e('Scan status', 'anmi-plugin-manager'); ?></span>
					<?php if (!empty($scan_result['safe'])): ?>
						<span class="badge badge-success"><?php esc_html_e('No threats detected', 'anmi-plugin-manager'); ?></span>
					<?php else: ?>
						<span class="badge badge-danger"><?php esc_html_e('Review required', 'anmi-plugin-manager'); ?></span>
					<?php endif; ?>
				</div>
			</div>
			...
			<div class="anmi-upload-actions">
				<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
					...
					<p class="description"><?php esc_html_e('Safe activate performs rollback checks and will revert automatically if errors are detected.', 'anmi-plugin-manager'); ?></p>

					<p class="description"><?php esc_html_e('Extracted files are placed into wp-content/anmi-staging-plugins before being copied into wp-content/plugins.', 'anmi-plugin-manager'); ?></p>
			...
			<?php if (!empty($metadata['description'])): ?>
				<p><?php echo esc_html(wp_trim_words($metadata['description'], 40)); ?></p>
			<?php endif; ?>
		</div>
	<?php endif; ?>
</div>
