---
title: "BT-MTB Morse Taper Tool Holder (Drawbar Nut Type)"
slug: "bt-mtb-morse-taper-nut"
primary_keyword: "bt-mtb morse taper"
tags: "BT-MTB, morse taper, tool holder, drawbar type, bầu kẹp dao phay côn, đuôi ren"
category: "BT SIDE LOCK"
catalog_page: "18"
seo_title: "BT-MTB Morse Taper Holder - MT1–MT5 (5 Sizes) - Bầu kẹp đuôi ren có ốc siết"
seo_description: "BT-MTB Morse Taper Holder của An Mi Tools. Loại kẹp bằng ốc siết (Drawbar), chuyên dùng cho dao phay ngón chuôi côn MT1-MT5. An toàn, độ đảo thấp."
social_image_facebook: ""
social_image_twitter: ""
seo_score_target: ">=70"
author: "An Mi Tools Technical Team"
date_published: "2025-10-20"
date_modified: "2025-11-27"
schema_type: "Product"
---

<section class="bt-mtb-morse-taper-nut">
    <h2>Tổng quan BT-MTB Morse Taper Tool Holder</h2>
    <p><strong>BT-MTB</strong> là bầu kẹp côn Morse thiết kế cho dao cụ có <strong>đuôi ren (Drawbar thread)</strong> thay vì đuôi dẹt. Sản phẩm sử dụng cơ cấu ốc siết bên trong (Bolt G) để kéo chặt dao vào bầu kẹp, đảm bảo độ cứng vững cao nhất, thường dùng cho dao phay ngón chuôi côn hoặc doa tinh.</p>

    <p>Tham khảo thêm về chuôi côn Morse: <a href="https://en.wikipedia.org/wiki/Morse_taper">Morse taper (Wikipedia)</a> — mô tả nguyên lý chuôi côn và các chuẩn kích thước liên quan.</p>

    <h3>Đặc điểm kỹ thuật</h3>
    <ul>
        <li><strong>Chuẩn kết nối:</strong> BT30, BT40, BT50.</li>
        <li><strong>Cơ cấu kẹp:</strong> Kẹp bằng bu lông rút (Drawbar Bolt) bên trong.</li>
        <li><strong>Ứng dụng:</strong> Dùng cho dao phay (Milling) hoặc Doa (Reaming) cần độ an toàn cao, tránh tuột dao khi rung động.</li>
        <li><strong>Ren kết nối (G):</strong> Tương thích M6 đến M20 tùy size.</li>
    </ul>

    <h3>Thông số kích thước (Specification)</h3>
    <table class="table-specs">
        <thead>
            <tr>
                <th>Model (BT)</th>
                <th>Mã sản phẩm (Spec)</th>
                <th>L (mm)</th>
                <th>Đường kính (D)</th>
                <th>Ren ốc siết (G)</th>
                <th>Trọng lượng (KG)</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td rowspan="2"><strong>BT30</strong></td>
                <td>MTB1-45</td>
                <td>45</td>
                <td>12.065</td>
                <td>M6 x 1</td>
                <td>0.55</td>
            </tr>
            <tr>
                <td>MTB2-45</td>
                <td>45</td>
                <td>17.780</td>
                <td>M10 x 1.5</td>
                <td>0.55</td>
            </tr>
            <tr>
                <td rowspan="4"><strong>BT40</strong></td>
                <td>MTB1-45</td>
                <td>45</td>
                <td>12.065</td>
                <td>M6 x 1</td>
                <td>1.05</td>
            </tr>
            <tr>
                <td>MTB2-45</td>
                <td>45</td>
                <td>17.780</td>
                <td>M10 x 1.5</td>
                <td>1.85</td>
            </tr>
            <tr>
                <td>MTB3-75</td>
                <td>75</td>
                <td>23.825</td>
                <td>M12 x 1.75</td>
                <td>1.30</td>
            </tr>
            <tr>
                <td>MTB4-90</td>
                <td>90</td>
                <td>31.267</td>
                <td>M16 x 2</td>
                <td>1.40</td>
            </tr>
            <tr>
                <td rowspan="5"><strong>BT50</strong></td>
                <td>MTB1-45</td>
                <td>45</td>
                <td>12.065</td>
                <td>M6 x 1</td>
                <td>3.8</td>
            </tr>
            <tr>
                <td>MTB2-45</td>
                <td>45</td>
                <td>17.780</td>
                <td>M10 x 1.5</td>
                <td>3.9</td>
            </tr>
            <tr>
                <td>MTB3-60</td>
                <td>60</td>
                <td>23.825</td>
                <td>M12 x 1.75</td>
                <td>4.0</td>
            </tr>
            <tr>
                <td>MTB4-75</td>
                <td>75</td>
                <td>31.267</td>
                <td>M16 x 2</td>
                <td>4.2</td>
            </tr>
            <tr>
                <td>MTB5-120</td>
                <td>120</td>
                <td>44.399</td>
                <td>M20 x 2</td>
                <td>4.3</td>
            </tr>
        </tbody>
    </table>
</section>